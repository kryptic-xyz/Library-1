local Kryptic = {}
Kryptic.Version = "1.3.0h"
Kryptic.Flags = {}
Kryptic.SaveCfg = false
Kryptic.Folder = "Kryptic"
Kryptic._CfgFile = ""
Kryptic.Binds = {}
Kryptic._BindListGui = nil
Kryptic._TopbarGui = nil
Kryptic._RadialGui = nil

Kryptic.ShowKeybindList = false
Kryptic.ShowTopbar = false
Kryptic.TopbarBind = nil
Kryptic.ShowRadial = false
Kryptic.RadialHotkey = nil
Kryptic.RadialMode = "hold"
Kryptic.RadialAnim = "Scale"

Kryptic._Tabs = {}
Kryptic.TabOrder = false
Kryptic._ElementRegistry = {}

Kryptic._ScriptKey = ""

Kryptic._MainWindowRef = nil
Kryptic._MinimizedRef = nil
Kryptic._RestoreRef = nil

Kryptic.OwnerButtons = {}
Kryptic.HoverMaximizeEnabled = true
Kryptic.HoverMaximizeDelay = 3 

Kryptic.UserSection = false 
Kryptic.UserSectionRightClick = false
Kryptic.USI = {}
Kryptic.UserSectionItems = Kryptic.USI
...
function Kryptic:Notify(configOrText, durationArg)
...
function Kryptic:CNotify(config)
	config = config or {}
...
local function saveFlags(folder, file)
	...
	pcall(function()
		...
		local saveData = {
			__version = Kryptic.Version,
			__gameId = game.GameId,
			__scriptKey = Kryptic._ScriptKey,
		}
		for key, flagObj in pairs(Kryptic.Flags) do
			if Kryptic.SaveCfg or flagObj.Save then
...
function Kryptic:AddOwnerButton(config)
	config = config or {}
	local buttonConfig = {
	...
	table.insert(Kryptic.OwnerButtons, buttonConfig)
	return buttonConfig
end


function Kryptic:ClearOwnerButtons()
	Kryptic.OwnerButtons = {}
end

function Kryptic:Topbar(theme)
	if not Kryptic.ShowTopbar then return end
	if Kryptic._TopbarGui and Kryptic._TopbarGui.Parent then
		Kryptic._TopbarGui:Destroy()
	end

	local tabs = Kryptic._Tabs
	local tabCount = #tabs
	...
	local ownerButtonCount = #Kryptic.OwnerButtons
	...
	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "KrypticTopbar"
	screenGui.ResetOnSpawn = false
	screenGui.DisplayOrder = 997
	screenGui.IgnoreGuiInset = true
	screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	secGui(screenGui)
	Kryptic._TopbarGui = screenGui
	...
	for tabIndex, tabEntry in ipairs(tabs) do
		...
		tabButton.MouseButton1Click:Connect(function()
			...
			if Kryptic._RestoreRef then pcall(Kryptic._RestoreRef) end
	...
	if ownerButtonCount > 0 then
		...
		table.sort(Kryptic.OwnerButtons, function(a, b) return a.Order < b.Order end)

		for idx, btnConfig in ipairs(Kryptic.OwnerButtons) do
	...
	local function buildIndex()
		local index = {}
		local seen = {}
		for flagName, flagObj in pairs(Kryptic.Flags) do
			local tabRef = nil
			for _, entry in ipairs(Kryptic._ElementRegistry) do
		...
		for _, bindEntry in ipairs(Kryptic.Binds) do
...
function Kryptic:Radial(theme)
	if not Kryptic.ShowRadial then return end
	if not Kryptic.RadialHotkey then
		warn("Kryptic >> ShowRadial = true but RadialHotkey is nil")
		return
	end
	if Kryptic._RadialGui and Kryptic._RadialGui.Parent then
		Kryptic._RadialGui:Destroy()
	end
	...
	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "KrypticRadial"
	screenGui.ResetOnSpawn = false
	screenGui.DisplayOrder = 999
	screenGui.IgnoreGuiInset = true
	screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	secGui(screenGui)
	Kryptic._RadialGui = screenGui
	...
	centerHint.Text = Kryptic.RadialMode == "hold" and "release\nto cancel" or "click\nto select"
	...
	local function openWheel()
		...
		local currentAnim = Kryptic.RadialAnim or "Scale"
	...
	if Kryptic.RadialMode == "toggle" then
	...
	UserInputService.InputBegan:Connect(function(input, gameProcessed)
		...
		if input.KeyCode ~= Kryptic.RadialHotkey then return end
		if Kryptic.RadialMode == "toggle" then
	...
	if Kryptic.RadialMode == "hold" then
...
function Kryptic:KeybindList(theme)
	if Kryptic.ShowKeybindList == false then return end
	if Kryptic._BindListGui and Kryptic._BindListGui.Parent then
		Kryptic._BindListGui:Destroy()
	end

	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "KrypticKeybindList"
	screenGui.ResetOnSpawn = false
	screenGui.DisplayOrder = 998
	screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	secGui(screenGui)
	Kryptic._BindListGui = screenGui
	...
	for _, entry in ipairs(Kryptic.Binds) do
		buildKeybindRow(entry)
	end

	local knownBindCount = #Kryptic.Binds
	task.spawn(function()
		while screenGui and screenGui.Parent do
			task.wait(0.5)
			if #Kryptic.Binds > knownBindCount then
				for newIndex = knownBindCount + 1, #Kryptic.Binds do
					buildKeybindRow(Kryptic.Binds[newIndex])
				end
				knownBindCount = #Kryptic.Binds
			end
		end
	end)
...
function Kryptic:Window(config)
	Kryptic._initDone = false
	config = config or {}

	local targets = {game:GetService("CoreGui"), LocalPlayer.PlayerGui}
	for _, target in pairs(targets) do
		for _, guiName in ipairs({"KrypticUI", "KrypticNotifications", "KrypticNotificationsClassic", "KrypticKeybindList", "KrypticTopbar", "KrypticRadial"}) do
	...
	table.clear(Kryptic.Binds)
	table.clear(Kryptic._Tabs)
	table.clear(Kryptic._ElementRegistry)
	...
	local windowName = config.Name or "KrypticUI"
	local theme = Themes[config.Theme] or Themes.Dark
	Kryptic._activeTheme = theme
	...
	if configScriptKey ~= "" then Kryptic._ScriptKey = configScriptKey end

	Kryptic.SaveCfg = doSaveConfig
	Kryptic.Folder = configFolder
	Kryptic._CfgFile = configFile
	...
	if screenGui.Parent then
		for _, child in pairs(screenGui.Parent:GetChildren()) do
			if child.Name == "KrypticUI" and child ~= screenGui then child:Destroy() end
		end
	end
	...
	Kryptic._MainWindowRef = mainWindow
	...
	toastScreenGui.Name = "KrypticToasts"
	...
	tabHolder.Size = UDim2.new(1, 0, 1, Kryptic.UserSection and -50 or 0)
	...
	if not Kryptic.UserSection then
		tabHolder.Size = UDim2.new(1, 0, 1, 0)
		bottomDivider.Visible = false
	end
	...
	bottomBar.Visible = Kryptic.UserSection
	...
	if Kryptic.UserSection then
		...
		local function openProfileView()
			...
			makeProfileBtn("Copy User ID", "rbxassetid://3944703587", function()
				if setclipboard then
					...
					Kryptic:Notify({Name = "Copied", Content = "User ID copied to clipboard.", Time = 3})
				end
			end)

			for _, item in ipairs(Kryptic.USI) do
		...
		avatarFrame.MouseButton2Click:Connect(function()
			if not Kryptic.UserSectionRightClick then return end
			openProfileView()
		end)
	...
	local function createMinimizedBar()
		...
		hoverDetector.MouseEnter:Connect(function()
			...
			hoverTimer = task.delay(Kryptic.HoverMaximizeDelay, function()
	...
	Kryptic._RestoreRef = restoreWindow
	Kryptic._MinimizedRef = function() return isMinimized end
	...
	function windowObject:SetAvatarMenuEnabled(enabled)
		Kryptic.UserSectionRightClick = enabled
	end

	function windowObject:AddAvatarMenuItem(name, callback, icon)
		table.insert(Kryptic.USI, {
			Name = name,
			Callback = callback,
	...
	function windowObject:Toast(text, icon)
		...
		task.spawn(function()
			local topY = Kryptic.ShowTopbar and (14 + 48 + 8) or 14
	...
	function windowObject:SetTheme(name)
		...
		if Kryptic._TopbarGui then Kryptic:Topbar(newTheme) end
		if Kryptic._RadialGui then Kryptic:Radial(newTheme) end
	end
	...
	function windowObject:Tab(tabConfig)
		...
		table.insert(Kryptic._Tabs, tabEntry)
		...
		function tabObject:Stepper(config, parentOverride)
			...
			stepObj:Set(cIndx)
			if stepFlag then Kryptic.Flags[stepFlag] = stepObj end
			...
			table.insert(Kryptic._ElementRegistry, {name = stepName, obj = stepObj, tab = tabEntry})
			return stepObj
		end
		...
		function tabObject:KeyValue(config, parentOverride)
			...
			table.insert(Kryptic._ElementRegistry, {name = kvName, obj = obj, tab = tabEntry})
			return obj
		end
		...
		function tabObject:Label(text, parentOverride)
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedText, obj = obj, tab = tabEntry})
			return obj
		end
		...
		function tabObject:Toggle(text, default, callback, flagName, parentOverride)
			...
			if resolvedFlag then Kryptic.Flags[resolvedFlag] = toggleObj end
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedText, obj = toggleObj, tab = tabEntry})
			return toggleObj
		end
		...
		function tabObject:TextBox(placeholder, callback, parentOverride)
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedPlaceholder, obj = obj, tab = tabEntry})
			return obj
		end
		...
		function tabObject:Slider(text, sliderMin, sliderMax, sliderDefault, callback, flagName, parentOverride)
			...
			if resolvedFlag then Kryptic.Flags[resolvedFlag] = sliderObj end
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedText, obj = sliderObj, tab = tabEntry})
			return sliderObj
		end
		...
		function tabObject:Paragraph(config, parentOverride)
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedTitle, obj = obj, tab = tabEntry})
			return obj
		end
		...
		function tabObject:Dropdown(config, parentOverride)
			...
			if dropdownFlag then Kryptic.Flags[dropdownFlag] = dropdown end
			...
			table.insert(Kryptic._ElementRegistry, {name = dropdownName, obj = dropdown, tab = tabEntry})
			return dropdown
		end
		...
		function tabObject:Bind(config, parentOverride)
			...
			if bindFlag then Kryptic.Flags[bindFlag] = bind end
			...
			table.insert(Kryptic._ElementRegistry, {name = bindName, obj = bind, tab = tabEntry})
			table.insert(Kryptic.Binds, {Name = bindName, Bind = bind, tab = tabEntry})
			return bind
		end
		...
		function tabObject:Colorpicker(config, parentOverride)
			...
			if colorFlag then Kryptic.Flags[colorFlag] = colorpicker end
			function colorpicker:Kryptic(t)
				headerNameLabel.TextColor3 = t.Text
			...
			table.insert(Kryptic._ElementRegistry, {name = colorName, obj = colorpicker, tab = tabEntry})
			return colorpicker
		end
		...
		function tabObject:ProgressBar(text, default, parentOverride)
			...
			table.insert(Kryptic._ElementRegistry, {name = resolvedText, obj = progressObj, tab = tabEntry})
	...
	task.defer(function()
    	if Kryptic._initDone then return end
    	Kryptic._initDone = true
    	if Kryptic.ShowKeybindList then Kryptic:KeybindList(theme) end
    	if Kryptic.ShowTopbar then Kryptic:Topbar(theme) end
    	if Kryptic.ShowRadial then Kryptic:Radial(theme) end
	end)
...
function Kryptic:Init(theme)
    if not Kryptic.SaveCfg then return end
    pcall(function()
        local configPath = Kryptic.Folder .. "/" .. Kryptic._CfgFile .. ".json"
        if isfile and isfile(configPath) then
            ...
            for key, savedValue in pairs(parsedData) do
                if key:sub(1, 2) ~= "__" then
                    if Kryptic.Flags[key] then
                        pcall(function() Kryptic.Flags[key]:Set(savedValue) end)
                    end
                end
            end
        end
...
function Kryptic:Watermark(title)
	if _wmGui and _wmGui.Parent then _wmGui:Destroy() end

	local theme = Kryptic._activeTheme or Kryptic.Themes.Dark

	local screenGui = Instance.new("ScreenGui")
	screenGui.Name = "KrypticWatermark"
	screenGui.ResetOnSpawn = false
...
Kryptic.MakeWindow = Kryptic.Window
Kryptic.MakeNotification = Kryptic.Notify

function Kryptic:Destroy()
	local targets = {game:GetService("CoreGui"), LocalPlayer.PlayerGui}
	for _, target in pairs(targets) do
		for _, guiName in ipairs({"KrypticUI", "KrypticNotifications", "KrypticNotificationsClassic", "KrypticKeybindList", "KrypticTopbar", "KrypticRadial"}) do
			local guiInstance = target:FindFirstChild(guiName)
	...
	table.clear(Kryptic._ElementRegistry)
	table.clear(notifStack)
	table.clear(Kryptic.OwnerButtons)
	Kryptic._BindListGui = nil
	Kryptic._TopbarGui = nil
	Kryptic._RadialGui = nil
	Kryptic._MainWindowRef = nil
